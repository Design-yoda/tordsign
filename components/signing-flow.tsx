    ...
    <div className="space-y-4 pb-24 xl:pb-0">
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <div className="flex flex-col gap-4">
          {/* Signer identity — hidden on mobile (shown in sticky footer instead) */}
          <div className="hidden rounded-[28px] border bg-white p-5 shadow-card xl:block">
      ...
      {/* Mobile sticky footer — always accessible on small screens */}
      <div className="fixed bottom-0 left-0 right-0 z-40 border-t bg-white px-4 pb-4 pt-3 shadow-[0_-4px_24px_rgba(0,0,0,0.08)] xl:hidden">
        <div className="flex items-center gap-2">
          <input
            value={signerName}
            onChange={(e) => setSignerName(e.target.value)}
            placeholder="Your full name to sign"
            className="h-10 min-w-0 flex-1 rounded-[6px] border border-slate-200 px-3 text-sm outline-none transition focus:border-ink"
          />
          <button
            type="button"
            onClick={submit}
            disabled={!canSubmit}
            className="shrink-0 rounded-full bg-ink px-4 py-2.5 text-sm font-semibold text-white transition disabled:bg-slate-200 disabled:text-slate-400"
          >
            {isPending ? "…" : "Sign"}
          </button>
        </div>
        ...
        ) : !canSubmit && !isPending && signerName.trim() ? (
          <p className="mt-1.5 text-center text-xs text-slate-500">
            {requiredFields.length - completedRequired} field{requiredFields.length - completedRequired !== 1 ? "s" : ""} remaining
          </p>
        ) : null}
      </div>
    </div>
